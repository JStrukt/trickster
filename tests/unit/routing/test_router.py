...
class TestRouteResponse:
    def test_deserialize_complete(self):
        ...
        assert response.body == {"works": True}
...
class TestRoute:
        ...
        route = Route.deserialize(
            {
                ...
                "responses": [{"id": "response_1", "body": {"works": True}}],
            }
        )
    ...
    def test_deserialize_minimal(self):
        route = Route.deserialize(
            {"id": "id1", "path": "/endpoint_\\w*", "responses": [{"body": ""}]}
        )
    ...
    def test_match_not_active(self):
        ...
        request = IncomingTestRequest(
            base_url="http://localhost/", full_path="/test_url", method="GET"
        )
    ...
    def test_match_active(self):
        route = Route(
            id="id1",
            responses=[RouteResponse("id1", "", Delay(), weight=0.4)],
            response_selection=ResponseSelectionStrategy.random,
            path=re.compile(r"/test.*"),
            auth=NoAuth(),
            method="GET",
...
class TestRouter:
    ...
    def test_match_route(self):
        ...
        request = IncomingTestRequest(
            base_url="http://localhost/", full_path="/endpoint", method="GET"
        )
        ...
        router.add_route(
            {
                "id": "id2",
                "path": "/endpoint\\w*",
                "responses": [{"id": "response_2", "body": {"works": True}}],
            }
        )
    ...
    def test_reset_router_with_default_routes(self):
        router = Router()
        router.reset(
            [{"path": "/minimal_endpoint", "responses": [{"body": "response"}]}]
        )
